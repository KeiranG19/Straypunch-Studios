...
public class Rigidbody_controller : MonoBehaviour {
	...
	void FixedUpdate()
	{
		...
		if (isGrounded)
		{
			...
			// By setting the isGrounded to false in every FixedUpdate we avoid
			// checking if the character is not isGrounded on OnCollisionExit()
			isGrounded = false;
		}
		// In mid-air
		else
	...
	// If there are collisions check if the character is isGrounded
	...
	public Vector3 CalculateVelocityChange(Vector3 inputVector)
	{
	...
	// Check if the base of the capsule is colliding to track if it's isGrounded
	private void TrackisGrounded(Collision collision)
	{
